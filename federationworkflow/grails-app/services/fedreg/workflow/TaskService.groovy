...
class TaskService {
	...
	def execute(TaskInstance taskInstance) {
		if(taskInstance.task.automated) {
			
		}
		else {
			if(taskInstance.status == TaskStatus.INITIATING) {
				// This is a non automated task which means we need to request permission from USERS || GROUPS || ROLES to proceed with execution
	...
	def reject(TaskInstance taskInstance, String reason) {
