...
<g:if test="${endpoints}">
  ...
  <g:each in="${endpoints.sort { it.id }}" status="i" var="ep">
    <div id="endpoint-${ep.id}" class="endpoint">
          ...
          <tr>
            <td colspan="2">
              <fr:hasPermission target="descriptor:${ep.descriptor.id}:endpoint:edit">
                <a class="edit-endpoint btn" data-id="${ep.id}" data-type="${endpointType}" data-container="${containerID}"><g:message code="label.edit"/></a>
              </fr:hasPermission>

              <fr:hasPermission target="descriptor:${ep.descriptor.id}:endpoint:toggle">
                <a class="confirm-toggle-endpoint btn" data-id="${ep.id}"><g:message code='label.togglestatus'/></a>
              </fr:hasPermission>
              ...
              <g:if test="${ep.instanceOf(aaf.fr.foundation.IndexedEndpoint) && !ep.isDefault}">
                <fr:hasPermission target="descriptor:${ep.descriptor.id}:endpoint:makedefault">
                  <a class="confirm-makedefault-endpoint btn" data-id="${ep.id}" data-type="${endpointType}" data-container="${containerID}"><g:message code='label.makedefault'/></a>
                </fr:hasPermission>
              </g:if>
