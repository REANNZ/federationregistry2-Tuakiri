...
<div class="fieldcontain ${hasErrors(bean: samlURIInstance, field: 'type', 'error')} ">
  ...
  <g:select name="type" from="${aaf.fr.foundation.SamlURIType?.values()}" keys="${aaf.fr.foundation.SamlURIType.values()*.name().encodeAsHTML()}" required="" value="${samlURIInstance?.type?.name().encodeAsHTML()}"/>
</div>
